  ...
  SwiftApiBuilder(super.pluginOptions, this.options, this.codecs) {
    ...
    _specs.add(
      SwiftClass(
        name: 'PlatformError',
        implements: ['Error'],
        fields: [requiredPlatformErrorField, ...optionalPlatformErrorFields],
        init: SwiftInit(
          parameters: [
            requiredPlatformErrorField.toInitParameter(label: '_'),
            ...optionalPlatformErrorFields.map(
              (e) => e.toInitParameter(label: '_', defaultTo: 'nil'),
            ),
          ],
        ),
        methods: const [
    ...
    const resultFields = [
      SwiftField(visibility: SwiftVisibility.private, name: 'result', type: 'FlutterResult'),
      SwiftField(visibility: SwiftVisibility.private, name: 'serializer', type: '(T) -> Any?'),
    ];
    _specs.add(
      SwiftClass(
        name: 'Result<T>',
        ...
        methods: const [
          SwiftMethod(
            name: 'success',
            parameters: [SwiftParameter(label: '_', name: 'data', type: 'T')],
            lambda: true,
            body: 'result(serializer(data))',
          ),
          SwiftMethod(
            name: 'error',
            parameters: [SwiftParameter(label: '_', name: 'error', type: 'PlatformError')],
    ...
    const controllerSinkFields = [
      SwiftField(visibility: SwiftVisibility.private, name: 'sink', type: 'FlutterEventSink'),
      SwiftField(visibility: SwiftVisibility.private, name: 'serializer', type: '(T) -> Any?'),
    ];
    _specs.add(
      SwiftClass(
        ...
        methods: const [
          ...
          SwiftMethod(name: 'endOfStream', lambda: true, body: 'sink(FlutterEndOfEventStream)'),
        ],
      ),
    );
  ...
  void writeHostApiClass(HostApiHandler handler) {
    ...
    _specs.add(
      SwiftProtocol(
        ...
        methods:
            methods.map((__) {
              final MethodHandler(element: e, swift: methodType) = __;
              ...
              return SwiftMethod(
                ...
                parameters: [
                  if (methodType == MethodApiType.callbacks)
                    SwiftParameter(label: '_', name: 'result', type: 'Result<$returnType>'),
    ...
    _specs.addAll(
      element.methods.where((e) => e.isHostApiEvent).map((e) {
        ...
        final parametersType = e.formalParameters.map(
          (e) => '_ ${e.displayName}: ${codecs.encodeType(e.type)}',
        );
        final parameters = e.formalParameters.mapIndexed(
            ...
            SwiftMethod(
              ...
              parameters: [
                SwiftParameter(
                  ...
                  annotation: 'escaping',
                  type:
                      '(${['_ sink: ControllerSink<${codecs.encodeType(returnType)}>', ...parametersType].join(', ')}) -> PlatformError?',
                ),
                SwiftParameter(
                  label: '_',
                  name: 'onCancel',
...
${switch (methodType) {
            ...
            try hostApi.${_encodeMethodName(e.displayName)}(${['res', ...parameters].join(', ')})''',
            MethodApiType.sync => '''
  ...
  void writeFlutterApiClass(FlutterApiHandler handler) {
    ...
    _specs.add(
      SwiftClass(
        name: codecs.encodeName(element.displayName),
        fields: const [SwiftField(name: 'channel', type: 'FlutterMethodChannel')],
        init: SwiftInit(
          parameters: [
        ...
        methods:
            methods.map((__) {
              ...
              return SwiftMethod(
                ...
                parameters:
                    e.formalParameters.map((e) {
                      return SwiftParameter(name: e.displayName, type: codecs.encodeType(e.type));
                    }).toList(),
                async: methodType == MethodApiType.async,
...
return try await withCheckedThrowingContinuation { continuation in
  ...
  void writeSerializableClass(SerializableClassHandler handler, {ClassElement? extend}) {
    ...
    if (children != null) {
      _specs.add(SwiftProtocol(name: codecs.encodeName(element.displayName)));
      if (flutterToHost) {
        _specs.add(
          SwiftMethod(
            name: 'deserialize${codecs.encodeName(element.displayName)}',
            parameters: [const SwiftParameter(label: '_', name: 'serialized', type: '[Any?]')],
    ...
    _specs.add(
      SwiftStruct(
        ...
        fields:
            params.map((e) {
              return SwiftField(name: _encodeVarName(e.name), type: codecs.encodeType(e.type));
            }).toList(),
        methods: [
          if (hostToFlutter)
            SwiftMethod(
  ...
  @override
  String build() =>
      '${SwiftEmitter().encode(SwiftLibrary(comments: const [generatedCodeComment], imports: const ['Flutter'], body: _specs))}';
