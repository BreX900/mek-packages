...
fun PaymentIntentStatus.toApi(): PaymentIntentStatusApi {
    return when (this) {
        ...
        // Added in Stripe Terminal 5.4 (reauthorization). PaymentIntentStatusApi has no
        // dedicated value, so map to REQUIRES_ACTION — "further action required" before
        // the intent can proceed (it is not captured/succeeded).
        PaymentIntentStatus.REQUIRES_REAUTHORIZATION -> PaymentIntentStatusApi.REQUIRES_ACTION
    }
}
